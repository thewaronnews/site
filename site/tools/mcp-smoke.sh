...
# tools/list (8 tools), every read tool, and suggest_correction without a
...
check "tools/list has 8 tools" "$R" "len(d['result']['tools'])==8"
...
R=$(call latest_news '{"limit":3}');                         check "latest_news" "$R" "'notes' in d['result']['structuredContent']"
