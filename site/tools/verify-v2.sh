#!/usr/bin/env bash
# v2 live verification (2026-09-22; /compare checks moved to /ladders in v3): every v2 route in HTML, .md and .json,
...
for p in / /incidents /search /countries /countries/us /countries/hu /continents /continents/europe /tactics /tactics/access_ban \
  /tactics/detention_and_violence /ladders /eras /eras/1970s /eras/2020s /leaders /leaders/donald-trump /coverage /terms /privacy \
...
for q in "/search?q=press+pass" "/search?q=white+house&country=US&tactic=access_ban" "/search?continent=europe&from=2020&to=2026" \
  ...
  "/incidents.json?continent=asia&has_case=0" "/incidents.md?tactic=funding_and_ownership_pressure&from=2025" "/ladders/access_ban?continent=north-america&from=2025" \
  "/ladders/access_ban.json" "/incidents?per_page=500"; do check "$q"; done
echo "== CSV =="
for q in /incidents.csv "/incidents?country=US&format=csv" "/search?q=pentagon&format=csv" "/ladders/access_ban?format=csv" "/countries/us?format=csv" "/tactics/access_ban?format=csv"; do check "$q"; done
